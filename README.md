...
--- 
все работает правильно, однако я не учел одного момента. Предположим пользователь послал файл file_0.jpg. Он сохранился в папку storage и записался в последний столбец базы данных. затем пользователь присылает еще один файл, например file_1.png. Он также будет сохранен в папку storage, однако в базе данных запись file_0.jpg в последней ячейке сотрется, и ВМЕСТО нее будет новая запись file_1.png. Такое может вызвать путаницу. Давай сделаем по другому, и изменим логику бота. Когда пользователь присылает файл он сохраняется н просто в папку /storage, а в новую, созданную специально под каждого пользователя папку, например /storage/user_files0, /storage/user_files1 и так далее, и уже в эти подкаталоги будут сохранятся все отправленные пользователем файлы. А в базу данных, в столбец planning_file мы будем записывать название не файла, а ПАПКИ куда сохранены пользовательские файлы, которые мы получили.