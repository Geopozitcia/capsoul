...
async def final_decision(message: types.Message, state: FSMContext):
    ...
    if nearest_day:
        ...
        await message.answer(
            ...
            f"Ближайший доступный день: {nearest_day_formatted}. Пожалуйста, выберите дату консультации:",
...
async def process_time(callback_query: CallbackQuery, state: FSMContext):
    ...
    await callback_query.message.answer(
        f"Ваша консультация назначена на {meeting_date} в {time}.\n"
        "\nПока что давайте перейдем к следующему шагу:", #TОDО - изменить этот высер
        reply_markup=ReplyKeyboardRemove()
    )
